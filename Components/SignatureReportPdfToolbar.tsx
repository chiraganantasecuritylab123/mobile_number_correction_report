...
export type SignatureReportPdfToolbarProps = {
  reportTitle?: string;
  reportDescription?: string;
  clientName?: string;
};

export default function SignatureReportPdfToolbar({
  reportTitle = "Signature Analysis Report",
  reportDescription = "HTML-to-PDF export with selectable text and preserved layout",
  clientName,
}: SignatureReportPdfToolbarProps) {
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      const { downloadSignatureReportPdfFromHtml } = await import(
        "@/lib/signature-report-pdf/htmlPdf"
      );
      await downloadSignatureReportPdfFromHtml(clientName);
    } catch (downloadError) {
      setError(
  ...
  const handlePreview = useCallback(async () => {
    ...
    setIsDownloading(true);
    try {
      const { previewSignatureReportPdfFromHtml } = await import(
        "@/lib/signature-report-pdf/htmlPdf"
      );
      const blob = await previewSignatureReportPdfFromHtml(clientName);
      ...
      setError(
        previewError instanceof Error ? previewError.message : "Failed to preview PDF.",
      );
    } finally {
      setIsDownloading(false);
    }
  }, [clientName, previewUrl]);
